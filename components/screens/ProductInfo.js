...
const ProductInfo = ({ route, navigation }) => {
  ...
  const scroollx = new Animated.Value(0);

  let position = Animated.divide(scroollx, width);
  ...
  const addToCart = (product) => {
    const checkBasket = cart.find((item) => item.id === product.id);
  ...
  return (
    ...
    >
        ...
        >
          ...
          <FlatList
            ...
            onScroll={Animated.event(
              [{ nativeEvent: { contentOffset: { x: scroollx } } }],
      ...
      >
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate("MyCart", { sepet: sepet })}
          style={{
            width: "40%",
            height: "90%",
            backgroundColor: COLOURS.blue,
